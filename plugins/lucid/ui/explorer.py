...
import ida_idaapi
import ida_kernwin
from lucid.microtext import (
    AddressToken,
    BlockNumberToken,
    MicrocodeOptions,
    MicrocodeText,
    MicroOperandToken,
    remap_mtext_position,
    translate_mtext_position,
)
...
from lucid.util.options import OptionListener
from lucid.util.python import CallbackHandler
from lucid.util.qt_shim import QObject, QtCore, QtWidgets, wrapinstance
    ...
    def activate_position(self, line_num, x, y):
        ...
        if isinstance(token, AddressToken):
            ...
            if isinstance(token, BlockNumberToken):
                ...
                isinstance(token, MicroOperandToken)
...
class MicrocodeExplorerModel(object):
    ...
    def rebuild_mtext(self, maturity):
        ...
        if maturity in self._rebuild_queue:
            ...
            self._refresh_queue[maturity] = (
                False  # no need to redraw since it was rebuilt
            )
        else:
            ...
            self.current_position = translate_mtext_position(
                self.current_position, old_mtext, new_mtext
            )
    ...
    def _gen_cursors(self, position, mmat_src):
        ...
        mmat_lower = [
            mmat for mmat in range(mmat_first, mmat_src) if mmat in mmat_levels
        ][::-1]

        # map the cursor forward from the source maturity
        mmat_higher = [
            mmat for mmat in range(mmat_src + 1, mmat_final + 1) if mmat in mmat_levels
        ]
    ...
    def _transfer_cursor(self, mmat_src, mmat_dst):
        ...
        projection = translate_mtext_position(
            position, self._mtext[mmat_src], self._mtext[mmat_dst]
        )

        # if translation failed, we will generate an approximate cursor
        if not projection:
            mapped = False
            projection = remap_mtext_position(
                position, self._mtext[mmat_src], self._mtext[mmat_dst]
            )
...
class MicrocodeExplorerView(
    ...
    def unload(self):
        ida_kernwin.close_widget(
            self._twidget, ida_kernwin.PluginForm.WCLS_DELETE_LATER
        )
        self._cleanup_hooks()
    ...
    def show(self):
        ...
        ida_kernwin.set_dock_pos(
            self.WINDOW_TITLE, "IDATopLevelDockArea", ida_kernwin.DP_RIGHT
        )
    ...
    def _ui_init_signals(self):
        ...
        # checkboxes
        self._checkbox_cursor.stateChanged.connect(
            lambda x: self.controller.set_highlight_mutual(bool(x))
        )
        self._checkbox_verbose.stateChanged.connect(
            lambda x: self.controller.set_option("verbose", bool(x))
        )
        self._checkbox_sync.stateChanged.connect(
            lambda x: self._code_sync.enable_sync(bool(x))
        )
        self._checkbox_devmode.stateChanged.connect(
            lambda x: self.controller.set_option("developer_mode", bool(x))
        )
