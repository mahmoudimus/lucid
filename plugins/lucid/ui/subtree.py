...
from lucid.util.qt_shim import QtCore, QtWidgets
...
class MOPHelper:
    ...
    def to_string(cls, mop):
        if mop.t == hr.mop_p:
            return (
                f"mop_pair<{cls.to_string(mop.pair.lop)},{cls.to_string(mop.pair.hop)}>"
            )
        elif mop.t == hr.mop_r:
...
class MicroSubtreeView(ida_graph.GraphViewer):
    ...
    def __init__(self, insn, controller):
        super(
            MicroSubtreeView,
            self,
        ).__init__(self.WINDOW_TITLE, True)
        self.insn = insn
        self.next_insn = None
    ...
    def update_insn(self):
        insn_token = self.controller.model.mtext.get_ins_for_line(
            self.controller.model.current_line
        )
    ...
    def show(self):
        ...
        ida_kernwin.set_dock_pos(
            self.WINDOW_TITLE, self.controller.view.WINDOW_TITLE, ida_kernwin.DP_BOTTOM
        )
    ...
    def _get_mop_oprops(self, mop):
        ...
        for oprop, name in [
            ...
            for x in filter(lambda y: y.startswith("OPROP_"), dir(ida_hexrays))
        ]:
            if mop.oprops & oprop:
                text += f" +{name[6:]}"
        return text
    ...
    def OnRefresh(self):
        ...
        self._node_color = (
            bg_color.blue() << 16 | bg_color.green() << 8 | bg_color.red()
        )
